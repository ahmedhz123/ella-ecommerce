<template>
  <div class="feats">
    <v-container fluid>
      <v-row>
        <v-col v-for="feat in features" :key="feat.title" class="px-2">
          <v-hover v-slot="{ props, isHovering }">
            <v-card
              :elevation="0"
              height="350"
              v-bind="props"
              :class="`mx-0 cursor-pointer d-flex flex-column justify-center bg-${
                isHovering ? 'indigo-lighten-4' : ''
              }`"
              :style="`transform: ${
                isHovering ? `translateY(-10px)` : ''
              }; transition: 0.5s`"
            >
              <div class="cont d-flex align-center flex-column">
                <div class="icon" v-html="feat.svg" :class="`mb-4`"></div>
                <v-card-title
                  class="font-weight-black"
                  style="font-size: 1rem"
                  >{{ feat.title }}</v-card-title
                >
                <v-card-text class="px-8 pt-5">{{ feat.desc }}</v-card-text>
              </div>
            </v-card>
          </v-hover>
        </v-col>
